...
class SitemapController extends Controller
{
    ...
    private function generateSitemap(): string
    {
        ...
        // Static pages
        $staticPages = [
            ['url' => route('courses.index'), 'priority' => '0.9'],
            ['url' => route('posts.categories'), 'priority' => '0.9'],
            ['url' => route('posts.index'), 'priority' => '0.8'],
            ['url' => route('students.register'), 'priority' => '0.8'],
        ];
        ...
        foreach ($courseCategories as $category) {
            $xml .= $this->addUrl(
                route('courses.category', $category->slug),
                $category->updated_at,
                'weekly',
                '0.8'
            );
        ...
        // Instructors
        $instructors = \App\Models\Instructor::where('status', 'active')
            ->whereNotNull('slug')
            ->select(['slug', 'updated_at'])
            ->get();
        ...
        foreach ($instructors as $instructor) {
            $xml .= $this->addUrl(
                route('instructors.show', $instructor->slug),
