...
class ServiceSeeder extends Seeder
{
    ...
    {
        // Kiểm tra xem model Service có tồn tại không
        if (!class_exists('App\Models\Service')) {
            $this->command->error('Service model not found. Please make sure the model is created first.');
            return;
        }

        // Kiểm tra xem bảng services có tồn tại không
        if (!\Illuminate\Support\Facades\Schema::hasTable('services')) {
            $this->command->error('Services table not found. Please run migrations first.');
            return;
        }
        $servicesData = [
            [
